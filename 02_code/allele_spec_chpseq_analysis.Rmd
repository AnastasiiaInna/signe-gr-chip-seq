...
write.csv2(dex.report, file = paste0(result.dir.fn, "dex_deseq_report.csv"))
dex.report
...
write.csv2(annotation.df, file = paste0(result.dir.fn, "dex_deseq_report_annotated.csv"))

anno.rasqual.df <- annotation.df[,c(113, 12:106)] # take only geneID and sampleIDs
write.csv2(anno.rasqual.df, file = paste0(result.dir.fn, "dex_deseq_anno_count_mtrx_for_rasqual.csv"))
```
...
Write BED files for each set of significant regions identified by DeSeq, separating them based on the gain or loss of enrichment. 

```{r, include=FALSE}
dex.enrich.df <- annotation.df  %>% filter(FDR < 0.1 & Fold > 0) 

write.table(dex.enrich.df, file = paste0(result.dir.fn, "dex_deseq_report_annotated_only_dex_echriched.csv"), sep = "\t", quote = F, row.names = F, col.names = F)

dex.enrich.anno.rasqual.df <- dex.enrich.df[,c(113, 12:106)] # take only geneID and sampleIDs
write.csv2(dex.enrich.anno.rasqual.df, file = paste0(result.dir.fn, "dex_deseq_anno_count_mtrx_for_rasqual_only_dex_enriched.csv"))
